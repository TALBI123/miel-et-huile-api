...
export class BackupsService {
    private static ALLTABLES :string[]  =  ["user","product", "category", "order","orderItem", "productImage", "productVariant"];
   ...
   static async getAllItems(tabName:keyof typeof prisma) {
    ...
    static async saveBackupToFile() {
        ...
        for(const table of this.ALLTABLES){
            ...
            const filePath = path.join(backupDir,`${table}_backup_${new Date().toISOString().split('T')[0]}.json`);
            if(fs.existsSync(filePath)){
                console.log(`Backup for table ${table} already exists at ${filePath}, skipping...`);
                continue;
            }
