...
const emailService_service_1 = require("./emailService.service");
...
class OrderProcessingService {
     ...
     */
    static async processOrderConfirmation({ orderId, email, customerName, session, order, }) {
        ...
        const confirmedOrder = await this.executeOrderConfirmationTransaction(orderId, session);
        console.log(`✅ Commande ${orderId} confirmée avec succès`);
        // Actions post-confirmation (non-bloquantes)
        this.executePostConfirmationActions(orderId, email, customerName, confirmedOrder);
        return confirmedOrder;
    }
    /**
     * Créer une nouvelle commande
     */
    static async createOrder(userId, cart, shippingCost) {
        ...
        const items = cart.map((item) => {
            ...
            if (!productVariant)
                throw new Error(`createOrder (OPS) :  Le produit avec l’ID ${item.variantId} n’existe plus. Veuillez choisir un autre variant.`);
        ...
        return prisma.$transaction(async (tx) => {
            ...
            const order = await tx.order.create({
                data: {
                    id: `cmd-${(0, helpers_1.generateToken)()}`,
                    userId,
                    totalAmount,
                    status: "PENDING",
     ...
     */
    static async executeOrderConfirmationTransaction(orderId, session) {
        const maxRetries = 3;
        let retryCount = 0;
        while (retryCount < maxRetries) {
            try {
                return await prisma.$transaction(async (tx) => {
                    ...
                    const [order] = await tx.$queryRawUnsafe(`SELECT * FROM "Order" WHERE id = $1 FOR UPDATE`, orderId);
                    if (!order)
                        throw new Error("Order not found");
                    if (order.status === "CONFIRMED")
                        return order; // ✅ déjà confirmé, rien à faire
                    ...
                    const updateOrder = await tx.order.update({
                        ...
                        data: {
                            ...
                            notes: `Traité par webhook session id ${session.id} à ${new Date().toISOString()}`,
                        },
                        include: {
                            items: {
                                include: {
                    ...
                    await inventory_service_1.InventoryService.decrementStock(updateOrder, tx);
                ...
                const isTransientError = transientErrors.some((word) => err.message.toLowerCase().includes(word));
                if (isTransientError && retryCount < maxRetries - 1) {
                    retryCount++;
                    const delay = Math.pow(2, retryCount) * 500; // backoff exponentiel
                    console.warn(`⚠️ Retry #${retryCount} après ${delay}ms pour cause : ${err.message}`);
                    await new Promise((res) => setTimeout(res, delay));
                }
                else {
                    // ❌ Erreur métier → pas de retry
                    throw new Error(`Transaction échouée après ${retryCount} tentative(s) : ${err.message}`);
                }
            }
        }
    }
     ...
     * Actions post-confirmation non-bloquantes
     */
    static async executePostConfirmationActions(orderId, email, customerName, order) {
        // Exécution asynchrone pour ne pas bloquer la réponse webhook
        this.runInBackground(async () => {
            try {
                // 1. Envoi email de confirmation
                await this.sendConfirmationEmailSafely(orderId, email, customerName, order);
                if (order.totalAmount > 1000) {
                    await this.notifyTeamLargeOrder(order);
                }
                // 3. Log de succès pour monitoring
                console.log(`🎉 Actions post-confirmation terminées pour commande ${orderId}`, {
                    orderId,
                    email,
     ...
     */
    static async createEmergencyOrder(session) {
        ...
        try {
            const emergencyOrder = await prisma.order.create({
                data: {
                    ...
                    paymentStatus: "PAID",
                    status: "REQUIRES_MANUAL_REVIEW", // Vous devrez ajouter ce statut à votre enum
                    stripePaymentIntentId: session.payment_intent,
     ...
     */
    static async findOrderByPaymentIntent(paymentIntentId) {
     ...
     */
    static getCustomerName(user, fallbackName) {
     ...
     */
    static async notifyTeamCriticalIssue(session, orderId) {
     ...
     */
    static async notifyTeamLargeOrder(order) {
        try {
            await (0, emailService_service_1.sendEmail)({
                to: process.env.ADMIN_EMAIL || "admin@votresite.com",
     ...
     */
    static async sendConfirmationEmailSafely(orderId, email, customerName, order) {
        try {
            const orderData = (0, object_1.createOrderData)({
                customerEmail: email,
    ...
    static runInBackground(fn) {
        setImmediate(() => {
            fn().catch((error) => console.error("Erreur background task:", error));
        });
    }
    static async resolveOrder(session) {
        const { orderId } = session.metadata || {};
        const paymentIntentId = session.payment_intent;
        // Essaye d'abord via paymentIntent
        let order = await OrderProcessingService.findOrderByPaymentIntent(paymentIntentId);
